...
from chunk_changelog import chunk_changelog_file
...
with open("data/raw_files.jsonl") as f:
    for line in f:
        ...
        if ftype == "code" and path.endswith(".py"):
            ...
            if path.upper().endswith("CHANGELOG.MD"):  # case-insensitive match
                all_chunks.extend(chunk_changelog_file(path, content, commit_sha, versions_per_chunk=5))
            else:
