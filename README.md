...
Ask natural-language questions about a real GitHub repository — its code, docs, and changelog — and get grounded answers with exact file:line citations. Built as an end-to-end RAG pipeline over encode/httpx, with a hand-built evaluation harness that measures retrieval and generation quality independently.
...
BM25 tokenizer fix — the regex tokenizer dropped digit-leading tokens ("0.28.1"), making version-number queries unretrievable.
Changelog re-chunking — the changelog was originally chunked one entry per version (60-100+ tiny, generic-sounding chunks), which crowded out unrelated queries in search. Re-chunked into groups of 5 versions per chunk, cutting the chunk count to 16 and eliminating most of the pollution.
